...
void HelloPointers()
{
	...
	void* invalidPtr = 0; // <- Ff we try to access this "memory address", the program WILL crash

	// How to store a memory address of a variable into a pointer:
	int num = 8;
	int* numPtr = &num; // Here, '&' gets the memory address of num, and we store it into numPtr
	...
	// double* badNumPtr = &num
	// However, we can cast the reference the pointer is pointing to to the correct type, like so:
	double* castedNumPtr = (double*)&num;
	std::cout << "castedNumPtr: " << castedNumPtr << std::endl;
	...
	// char** doublePtr = &ptr; <- This is bad
	void** doublePtr = &ptr; // <- This is good
	std::cout << "doublePtr: " << doublePtr << std::endl;
	std::cout << "Dereferenced doublePtr: " << *doublePtr << std::endl;
	...
	std::cout << "Dereferenced numPtr: " << *numPtr << std::endl;

	(*numPtr)++; // This increments the variable that the pointer is pointing at (num)
	// Because of order of operations, we needed to enclose *numPtr in '()' in order to increment the variable it's pointing at

	std::cout << "Changed dereferenced numPtr: " << *numPtr << std::endl;
...
// int incrementMe = 5;
// incrementMe = IncrementByValue(incrementMe)
// 
// This is bad and inefficient, not just because it looks horrendus, but also because it's slow;
// It needs to copy the value of 'incrementMe', put it in a new variable 'x', return that variable and set 'incrementMe' to it, and finally delete 'x'
...
void HelloReferences()
{
	...
	int incrementMe = 1;
	IncrementByValue(incrementMe);
	std::cout << incrementMe << std::endl;
	IncrementByReference(incrementMe);
	std::cout << incrementMe << std::endl;
